...
>
> 所以策略是**如实报告碎片，而不是假装解决了它**——上位工具能说「总空闲 12K，
> 但最大连续对齐块 8K，这个 MDL 需要 8K 对齐，装不下；建议先卸载 X」。
> 一个说得清为什么装不下的分配器，比一个偶尔神秘失败的好得多。
...
| # | 任务 | 说明 |
|:---:|---|---|
| S1 | 多槽位 arena 与对齐分配器 | 每槽位的 text/data/heap/stack 必须 2 的幂且自然对齐（MPU 硬性要求）。链接脚本改为预留 N 份对齐区间 |
